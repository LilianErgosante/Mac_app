...
if image_path:
    ...
    replace_photo_tag_with_image("CR_lombaire2.docx", image_path, new_doc_path)
...
for column in zip(*fourth_tables_data):
    column_average4 = mean(column)
    column_averages4.append(column_average4)
    
column_averages14 = []
for column in zip(*fourteenth_tables_data):
    column_average14 = mean(column)
    column_averages14.append(column_average14)
    
# enregistrement des données
second_number = round(column_averages4[1], 1)
third_number = round(column_averages4[2], 0)+round(column_averages4[3], 1)+round(column_averages4[4], 0)
rula_back = round(column_averages14[1], 0) + round(column_averages14[2], 0)
...
font = FontProperties(fname=font_path, size=18, weight='bold')

fig, ax = plt.subplots()
ax.pie(column_averages4, autopct='%1.1f%%', colors=mycolors, explode=explode)
ax.set_title('Flexion de tronc', fontproperties=font, fontsize=20)
...
ax.bar(y_pos, column_averages14[:3], align='center', color=mycolors2)
ax.set_xticks(y_pos)
ax.set_xticklabels(scores, fontproperties=font)
ax.set_title('Score RULA du tronc', fontproperties=font, fontsize=20)

# Ajout des étiquettes de valeur pour chaque barre
for i, v in enumerate(column_averages14[:3]):
...
plt.savefig('rula.png', format='png', dpi=300)
...
def generer_document():
    ...
    balises = {
        ...
        "##pauses##": pauses,
        "##poste##": post,
        "##pourcentage##": str(second_number),
        "##pourcentagesup##": str(third_number),
        "##rula_pourcentage##": str(rula_back),
    }

    remplacer_balises(document, balises)
    
    graphique_balise = "##graphique_flexion_tronc##"
    graphique_balise2 = "##rula_tronc##"
    ...
    for paragraph in document.paragraphs:
        if graphique_balise in paragraph.text:
            ...
            run.add_picture("graphique.png", width=Inches(6))
            break
    ...
    for paragraph in document.paragraphs:
        if graphique_balise2 in paragraph.text:
            ...
            run.add_picture("rula.png", width=Inches(6))
            break
